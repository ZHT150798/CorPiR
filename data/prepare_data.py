...
def resize_worker(img_file, sizes, resample, lmdb_save=False):
    ...
    img = img.convert('RGB')
    #img = crop_and_convert(img, sizes[1])  # 不直接缩放，先裁切
    out = resize_multiple(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', '-p', type=str, required=True,
                        help='folder with the source images')
    parser.add_argument('--out', '-o', type=str, required=True,
                        help='output folder; "_<low>_<high>" is appended to this name')

    parser.add_argument('--size', type=str, default='32,128',
                        help='<low>,<high> resolution, e.g. 32,128 for x4')
