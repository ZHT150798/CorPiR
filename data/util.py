...
hflip_v1 = torchvision.transforms.RandomHorizontalFlip(p=1)
Vflip_v1 = torchvision.transforms.RandomVerticalFlip(p=1)
...
def transform_augment(img_list, split='val', min_max=(-1, 1)):
    ...
    return imgs
def transform_augment_v1(img_list, split='val', min_max=(0, 1)):
    imgs = [totensor(img) for img in img_list]
    if split == 'train':
        p = random.random()
        for i in range(len(imgs)):
            if p < 0.5:
                imgs[i] = hflip_v1(imgs[i])
